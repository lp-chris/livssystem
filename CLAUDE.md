...
### 2026-07-02
- **«Nylig fanget» tilbake på hjemskjermen** — Sikkerhetsnettet for AI-rutingen (fangst-korreksjon fra 2026-06-23) var foreldreløst: `SisteFangster`/`FangstSeksjon` ble ikke rendret av noen side, så feilrutede fangster var usynlige — i strid med ruting-filosofien («hver fangst er synlig i 'nylig lagt til'»). Hjemskjermen (`(app)/page.tsx`) henter nå de 5 siste `captures` og rendrer `<SisteFangster init={fangster} />` nederst. `SisteFangster.tsx` gjort prop-drevet (server-data inn, synkes via `useEffect` når `router.refresh()` kjører etter ny fangst) i stedet for å hente selv. Dødkode slettet: `FangstSeksjon.tsx`, `FangstSkjema.tsx` (gammelt pre-overlay fangst-UI) og `api/capture/siste/` (eneste konsument var SisteFangster).
- **AI-ruteren vet nå dagens dato** — `RUTING_PROMPT` var statisk, så «i morgen» og «på fredag» ga gjettede forfallsdatoer. `rutMedAI` i `api/capture/route.ts` legger nå dagens dato + ukedag (Europe/Oslo) i system-prompten ved hvert kall.
- **UTC-datofeil fikset i hele appen** — Serveren kjører i UTC, så `new Date().toISOString().split("T")[0]` ga gårsdagens dato mellom midnatt og kl. 01/02 norsk tid: rutine-avhukinger ble lagret på feil dato (ødela streaks), «det som haster» og forfall-etiketter bommet, og kalender-vinduet dekket feil døgn. Alle forekomster byttet til `iDagOslo()`/`datoOslo()` fra `lib/dato.ts` — både sider (`rutiner`, `oppgaver`, `journal/alle`, hjem), API-ruter som *skriver* datoer (`api/rutiner/[id]/fullfor`, `api/rutiner`, `api/oppgaver`, `api/oppgaver/[id]` tilbakevendende-forfall, `api/capture` rutine-startDate) og klientkomponenter (`OppgaveKort`, `RutineKort`, `DetSomHaster`). To nye hjelpere i `lib/dato.ts`: `timeOslo()` (riktig time til «God morgen»-hilsenen) og `osloOffset()` (RFC3339-offset til Google Calendar-vinduet i `googleKalender.ts`, som nå spør etter `T00:00:00+02:00`–`T23:59:59+02:00` norsk tid i stedet for UTC-døgnet).
- **Fangst-kvittering: journal-etikett** — `FangstOverlay` viste «Element» for journal-fangster; `journal: "Journal"` lagt til i `RUTETYPE_ETIKETT`.
